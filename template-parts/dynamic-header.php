	...
	<div class="container px-0">
		<div class="site-branding show-<?php echo esc_attr(hello_elementor_get_setting('hello_header_logo_type')); ?>">
			...
			if ($site_name && ('logo' !== hello_elementor_get_setting('hello_header_logo_type') || $is_editor)) : ?>
				<h1 class="site-title <?php echo esc_attr(hello_show_or_hide('hello_header_logo_display')); ?>">
					<a href="<?php echo esc_url(home_url('/')); ?>" title="<?php echo esc_attr__('Home', 'hello-elementor'); ?>" class="reset-link listen-theme-color" rel="home">
		...
		<?php if ($header_nav_menu) : ?>
			<nav class="site-navigation croxo-font-text--deep <?php echo esc_attr(hello_show_or_hide('hello_header_menu_display')); ?>">
				...
				?>
				<?php
				//ABOUT MENU ITEM QUERY
				// Create a new WP_Query to retrieve a single post from your custom post type
				$custom_post_args = array(
					'post_type' => 'about_text', // Your custom post type slug
					'posts_per_page' => 1, // Limit to 1 post
					// Add more parameters if needed (e.g., post ID, specific taxonomy filters, etc.)
				);
				...
				?>
						<li id="croxoAboutMenuItem" class="menu-item menu-item-object-page menu-item--about d-flex flex-column">
							<span class="menu-item--about__title"><?php the_title(); ?></span>
							<div class="menu-item--about__text croxo-color-transparent croxo-background-transparent--deep--imporant p-events-none--deep">
								<?php the_content(); ?>
							</div>
						</li>
				<?php
